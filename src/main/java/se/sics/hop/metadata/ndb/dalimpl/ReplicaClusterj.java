...
import se.sics.hop.metadata.tabledef.ReplicaTableDef;
...
public class ReplicaClusterj implements ReplicaTableDef, ReplicaDataAccess<HopIndexedReplica> {
  ...
  @Override
  public List<HopIndexedReplica> findReplicasById(long id) throws StorageException {
    try {
  ...
  private List<HopIndexedReplica> createReplicaList(List<ReplicaDTO> triplets) {
    ...
    for (ReplicaDTO t : triplets) {
      replicas.add(new HopIndexedReplica(t.getBlockId(), t.getStorageId(), t.getIndex()));
    }
    return replicas;
  }
